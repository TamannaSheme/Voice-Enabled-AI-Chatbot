...
describe('Voice Enabled Chatbot UI', () => {
  ...
  test('should have an input and button for sending', () => {
  ...
  test('should append user input to chat box when sendMessage is called', () => {
    const input = container.querySelector('#user-input');
    input.value = "Hello";
    const chatBox = container.querySelector('#chat-box');
    global.sendMessage();
    expect(chatBox.innerHTML).toMatch(/Hello/);
  });
});
